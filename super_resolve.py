...
def visualize_sr(img, halve=False):
    ...
    sr_img_srresnet = srresnet(
        convert_image(lr_img, source='pil',
                      target='imagenet-norm').unsqueeze(0).to(device))
    sr_img_srresnet = sr_img_srresnet.squeeze(0).cpu().detach()
    sr_img_srresnet = convert_image(sr_img_srresnet,
                                    source='[-1, 1]',
                                    target='pil')
    ...
    margin = 40
    grid_img = Image.new(
        'RGB', (2 * hr_img.width + 3 * margin, 2 * hr_img.height + 3 * margin),
        (255, 255, 255))

    # Font
    draw = ImageDraw.Draw(grid_img)
    try:
        font = ImageFont.truetype("./fonts/Roboto-Regular.ttf", size=24)
    ...
    draw.text(xy=[
        ...
        text_size[0] / 2, margin - text_size[1] - 5
    ],
              text="SRResNet",
              font=font,
              fill='black')
...
if __name__ == '__main__':
    grid_img = visualize_sr("./data/test/deno.jpg")
